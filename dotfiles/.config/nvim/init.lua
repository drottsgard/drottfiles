...

  {
    ...
    config = function()
      ...
      require('nvim-treesitter').install({
        ...
        'astro', 'css', 'markdown', 'markdown_inline',
      })
...
-- Treesitter highlight and indent are now built-in to Neovim.
-- Configuration was moved to the plugin spec above.
...
local servers = {
  ...
  tailwindcss = {
    settings = {
      tailwindCSS = {
        experimental = {
          -- one pattern per entry; escape the \ in Lua strings
          classRegex = {
            -- 1️⃣ object‑style  →  const styles = { key: '…' }
            ':\\s*[\'"`]([^\'"`]*)[\'"`]',
            -- 2️⃣ assignment    →  const Heading = '…'
            '=\\s*[\'"`]([^\'"`]*)[\'"`]',
          },
        },
        -- If you like the “…Styles” variable trick as well:
        -- classAttributes = { 'class', 'className', 'classList', '.*Styles' },
      },
    }
  },
  jsonls = {},
...
-- Ensure the servers above are installed and configured
...
vim.api.nvim_create_autocmd('LspAttach', {
  callback = function(args)
    ...
    -- Run the general on_attach
    on_attach(client, bufnr)
    -- Run server-specific on_attach if defined
    local server_name = client and client.name
    local server_opts = server_name and servers[server_name]
    if server_opts and server_opts.on_attach then
...
vim.keymap.set('n', '<leader>yf', function()
  ...
  if path:find(config_dir, 1, true) == 1 then
    vim.fn.setreg('+', 'nvim/' .. path:sub(#config_dir + 2))
  else
    vim.fn.setreg('+', vim.fn.expand('%'))
  end
end, { desc = '[y]ank filename' })
vim.keymap.set('n', '<leader>cb', ':bd <CR>', { desc = '[c]lose [b]uffer' });

-- Autocmd to populate .tsx files with boilerplate content
